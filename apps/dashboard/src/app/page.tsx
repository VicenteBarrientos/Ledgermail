...
import { 
  Mail, Shield, RefreshCw, FileText, CheckCircle, AlertTriangle, 
  XCircle, Coins, Clock, Database, ChevronRight, Play, Server, 
  Settings, Info, BarChart3, HelpCircle 
} from "lucide-react";
...
export default function Dashboard() {
  ...
  const [activeTab, setActiveTab] = useState<"json" | "html" | "attempts" | "replay">("json");
  ...
  const filteredTransactions = transactions.filter(txn => {
    const matchesSearch = 
      txn.senderName?.toLowerCase().includes(search.toLowerCase()) ||
        ...
        <section className="lg:col-span-2 glass-card rounded-xl p-4 flex flex-col gap-4 max-h-[600px] overflow-hidden">
          <div className="flex gap-2">
            <input 
              ...
              className="flex-1 text-xs px-3 py-2 rounded bg-slate-900 border border-[rgba(255,255,255,0.08)] text-slate-200 focus:outline-none focus:border-indigo-500"
            />
            <select
              value={statusFilter}
              onChange={e => setStatusFilter(e.target.value)}
              className="text-xs px-3 py-2 rounded bg-slate-900 border border-[rgba(255,255,255,0.08)] text-slate-200 focus:outline-none"
            >
          ...
          <div className="flex-1 overflow-y-auto flex flex-col gap-2">
            {filteredTransactions.length === 0 ? (
              <div className="text-center py-8 text-xs text-slate-500">No transactions match filters.</div>
            ) : (
                  ...
                  >
                    <div className="flex justify-between items-center">
                      ...
                      <span className={`px-2 py-0.5 rounded text-[9px] font-semibold ${
                        status === "PARSED" ? "bg-green-500/10 text-green-400" :
        ...
        <section className="lg:col-span-3 glass-card rounded-xl p-5 flex flex-col gap-4 min-h-[500px]">
          {selectedTxn ? (
            <>
              ...
              <div className="flex gap-2 border-b border-[rgba(255,255,255,0.06)] pb-2">
                ...
                <button
                  ...
                  className={`text-xs font-semibold px-3 py-1.5 rounded transition ${
                    activeTab === "attempts" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-slate-200"
                  }`}
                >
                ...
                <button
                  ...
                  className={`text-xs font-semibold px-3 py-1.5 rounded transition ${
                    activeTab === "replay" ? "bg-indigo-600 text-white" : "text-slate-400 hover:text-slate-200"
                  }`}
                >
                  <Play className="w-3.5 h-3.5 inline mr-1.5 text-indigo-400" />
                  Replay Mode
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto max-h-[350px]">
                {activeTab === "json" && (
                  <pre className="p-4 rounded-lg bg-slate-950 text-xs text-indigo-300 font-mono overflow-x-auto leading-relaxed border border-[rgba(255,255,255,0.04)]">
                    {JSON.stringify({
                      id: selectedTxn.id,
                ...
                {activeTab === "html" && (
                  <div className="p-4 rounded-lg bg-slate-950 text-xs font-mono text-slate-300 overflow-x-auto border border-[rgba(255,255,255,0.04)] max-h-[300px]">
                ...
                {activeTab === "attempts" && (
                  <div className="flex flex-col gap-4">
                    {selectedTxn.attempts?.map((att: any, idx: number) => (
                      <div key={att.id || idx} className="p-4 rounded-lg bg-slate-950/60 border border-[rgba(255,255,255,0.04)] flex flex-col gap-3">
                        <div className="flex justify-between items-center border-b border-[rgba(255,255,255,0.06)] pb-2">
                          ...
                          <span className={`px-2 py-0.5 rounded text-[10px] font-semibold ${
                            att.success ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"
                          }`}>
                        ...
                        {att.validationErrors && (
                          <div className="mt-2 p-2.5 rounded bg-red-950/20 border border-red-500/15 text-[10px] text-red-400 leading-relaxed font-mono">
                ...
                {activeTab === "replay" && (
                  <div className="flex flex-col gap-4 p-4 rounded-lg bg-slate-950/60 border border-[rgba(255,255,255,0.04)]">
                      ...
                      <div>
                        ...
                        <select 
                          ...
                          onChange={e => {
                            ...
                            if (e.target.value === "openai") setReplayModel("gpt-4o-mini");
                            else if (e.target.value === "gemini") setReplayModel("gemini-1.5-flash");
                            else setReplayModel("claude-3-5-sonnet-20241022");
                          }}
                    ...
                    <button 
                      ...
                      disabled={isLoading}
                      className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-indigo-800 transition text-xs font-semibold rounded-lg text-white mt-4 flex items-center justify-center gap-1.5"
                    >
            ...
            <div className="flex-1 flex flex-col justify-center items-center text-slate-500">
              <Mail className="w-12 h-12 mb-3 text-slate-600 animate-bounce" />
